...
**RabbitMQ** is the message broker used for communication between all the internal services. It existst to decouple all the plugin-parts of Musibot from the core parts (*Orchestrators* and *Workers*), as well as providing a producer-consumer pattern implementation between *Pipelines* and *Models*. It also carries what *Workers* and *Orchestrators* have to say while they work — the log lines they produce, and the *Files* they have just written — up to the *Web API*, which forwards both over SSE to whoever is watching that page in the *Web UI*, so that a *User* sees a reading happen rather than waiting on it.
