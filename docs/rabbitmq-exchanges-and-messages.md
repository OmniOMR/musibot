...
Queues are declared by the consumers, named after what they consume (`musibot.pipeline.hello-world@1.0.0`), and auto-delete when their last consumer disconnects, so a departed *Orchestrator* leaves nothing behind to accumulate messages. Nothing survives a broker restart: all of Musibot's state is ephemeral, and a broker restart is a system restart.

The name and the declaration flags of a shared work queue are part of this protocol rather than of any one head, and live as constants in `musibot.core.execution` — `model_work_queue()` and `pipeline_work_queue()`. RabbitMQ is what makes them protocol: a second declaration disagreeing with the first is refused with `PRECONDITION_FAILED` and takes the channel with it, so a *Worker Head* and an *Orchestrator Head* cannot each decide for themselves. The queues belonging to a single process — reply queues and control queues — are named where they are declared, since nobody else can be wrong about them.

> **Note:** The shared work queues are nonetheless declared *durable*, which is a concession to RabbitMQ rather than a change of intent. RabbitMQ 4 refuses a transient queue that is not exclusive, and a work queue cannot be exclusive — being shared by every competing consumer is the whole point of it. The ephemerality that matters is preserved anyway: every message is published non-persistent, so a broker restart still discards everything in flight, and the queue itself still auto-deletes with its last consumer. Queues belonging to a single process — the `api` service's result, discovery and reply queues, and each head's control queue — are exclusive, and so stay transient.
