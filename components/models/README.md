...
- **[dvorak-ola](dvorak-ola/)** — page layout detection; reads `image.jpg` and writes a Musicorpus `layout.json` of staves, systems, grandstaves and their measures. The recognition is [an external YOLO checkpoint](https://github.com/v-dvorak/omr-layout-analysis) and what lives here is the wrapper around it, which is the other shape a model in this folder takes. It is also **the one component of Musibot that is not Apache-2.0** — it imports ultralytics, which is AGPL-3.0, and [its README](dvorak-ola/README.md#licensing) works through what that means for a publicly reachable instance. Read that before deploying it as an online service.
