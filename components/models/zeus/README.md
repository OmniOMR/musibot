...
**The model's log.** Zeus's stdout — including the per-execution failures written for a human to read — is captured by the head, published onto `musibot.logs`, and streamed by the `api` service to whoever is watching that page, so a failed transcription explains itself in the *Web UI*'s recognition log. It is also in the worker's journal, which is where to look for anything printed while no page was being watched — a model loading its weights, or a failure nobody had the page open for.
