...
- The SSE streams are driven as raw ASGI (`tests/streaming.py`) rather than through `TestClient`, because a stream that never ends is what a buffering test client cannot read.
