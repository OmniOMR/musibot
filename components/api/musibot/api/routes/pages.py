...
def delete_page(request: Request, page: MusicorpusPage = Depends(get_owned_page)) -> None:
    ...
    Removing it from the repository is what makes it vanish from the API. The
    rest of its resources — the MinIO folder, any running *Pipeline Execution* —
    are freed by collaborators wired in as those integrations land.
    """
    repository: MusicorpusPageRepository = request.app.state.pages
