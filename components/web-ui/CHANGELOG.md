...
Everything the UI is actually for. There is no upload, no pipeline picker, no progress view and no results view — `src/App.tsx` is a placeholder swatch that exists so the theme can be looked at. The API client layer is not written either.

Nor is the deployment. The instance is served under a `/musibot/` path prefix rather than at the root of a host, which the build has to be told about; the nginx configuration and the object storage arrangement behind it are being settled separately.
