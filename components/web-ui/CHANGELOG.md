...
Seeing the results rather than downloading them. The MusicorpusPage screen lists what a page holds, but its three other panels are not built: there is no canvas showing the scan and the boxes found on it, no rendered notation beside it, and no recognition log — the log pill opens nothing, and the API has no log endpoint to open it onto. A page also cannot yet have a further *Pipeline* run on it from the screen, so the *+ Run pipeline* button is absent rather than dead.
